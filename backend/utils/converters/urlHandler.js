...
/**
 * 将 GitHub 页面链接转为 raw 直链（Subconverter config 需要可下载地址）
 */
function normalizeTemplateUrl(url) {
  ...
  try {
    ...
    if (u.hostname !== 'github.com') return trimmed;
...
/**
 * 将多行节点链接格式化为 Subconverter url 参数
 * Subconverter 合并多个订阅/节点时使用 | 分隔（换行仅会被解析为第一个节点）
 */
function formatNodesForSubconvertUrl(nodeContent) {
  return nodeContent
    .trim()
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function buildSubconvertApiUrl(subconvertUrl, subscriptionUrl, targetFormat, customTemplateUrl = null, directUrl = null) {
  ...
  const normalizedTemplate = normalizeTemplateUrl(customTemplateUrl);
  if (normalizedTemplate) {
    requestParams.set('config', normalizedTemplate);
